...
if [ ${DEPLOY_ENV} == 'stage' ]; then
    ...
    if [ $(ssh_exec "docker exec ${DEPLOY_PATH##*/}-${PROD_PROJECT} drush status bootstrap 2> /dev/null | grep -c Successful") == 1 ]; then
        ...
        ssh_exec "perl -pi -e \"s/CHANGE MASTER TO MASTER_USE_GTID=slave_pos;/CHANGE MASTER TO MASTER_HOST='${DEPLOY_PATH##*/}-${PROD_PROJECT}.mariadb', MASTER_USE_GTID=slave_pos;/ if $. == 34\" dump.sql"
